...
bool metal_init_frame_debugging_impl(void) {
    metal_debug_log("Initializing frame debugging...");
    
    if (!ctx || !ctx->device) {
        metal_debug_log("ERROR: Cannot initialize frame debugging - ctx=%p, device=%p", ctx, ctx ? ctx->device : NULL);
        return false;
    }
    ...
    metal_debug_log("Frame debugging initialized successfully - auto-capture timeout: %.1f seconds", ctx->autoCaptureTimeoutSecs);
    return true;
}
