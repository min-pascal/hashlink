...
// New function: Render compute cubes into an existing command buffer
// Returns a new encoder handle that replaces the old one
// This properly integrates with MetalDriver's rendering flow
HL_PRIM vdynamic* HL_NAME(render_compute_cubes_with_encoder)(vdynamic *cmdBuffer, vdynamic *encoder) {
    if (!ctx || !ctx->device || !ctx->computeRenderPipelineState) {
        metal_debug_log("ERROR: render_compute_cubes_with_encoder() - context not initialized");
        return NULL;
    }
    
    if (cmdBuffer == NULL || encoder == NULL) {
        metal_debug_log("ERROR: render_compute_cubes_with_encoder() - null command buffer or encoder");
        return NULL;
    }
    
    if (!ctx->currentDrawable) {
        metal_debug_log("ERROR: render_compute_cubes_with_encoder() - no current drawable");
        return NULL;
    }

    @autoreleasepool {
        id<MTLCommandBuffer> commandBuffer = (__bridge id<MTLCommandBuffer>)cmdBuffer;
        id<MTLRenderCommandEncoder> oldRenderEncoder = (__bridge id<MTLRenderCommandEncoder>)encoder;
        ...
        // STEP 1: End the current render encoder so we can use a compute encoder
        [oldRenderEncoder endEncoding];
        
        // STEP 2: Run compute shader to generate Mandelbrot texture
        if (ctx->textureAnimationBuffer) {
            uint *animPtr = (uint*)[(__bridge id<MTLBuffer>)ctx->textureAnimationBuffer contents];
            *animPtr = (ctx->animationIndex++) % 5000;
            [(__bridge id<MTLBuffer>)ctx->textureAnimationBuffer didModifyRange:NSMakeRange(0, sizeof(uint))];
        ...
        // STEP 3: Update instance and camera data
        id<MTLBuffer> instanceBuffer = (__bridge id<MTLBuffer>)ctx->computeInstanceDataBuffers[ctx->frameIndex];
        struct metal_lighting_instance_data *instanceData = (struct metal_lighting_instance_data *)[instanceBuffer contents];
        ...
        // STEP 4: Get the existing drawable and depth texture
        ...
        // STEP 5: Create a NEW render pass that LOADS existing content (preserves clear color)
        MTLRenderPassDescriptor *renderPassDescriptor = [MTLRenderPassDescriptor renderPassDescriptor];
        renderPassDescriptor.colorAttachments[0].texture = drawable.texture;
        renderPassDescriptor.colorAttachments[0].loadAction = MTLLoadActionLoad; // LOAD to preserve background
        renderPassDescriptor.colorAttachments[0].storeAction = MTLStoreActionStore;
        ...
        // STEP 6: Create NEW render encoder for the compute cubes
        id<MTLRenderCommandEncoder> newRenderEncoder = [commandBuffer renderCommandEncoderWithDescriptor:renderPassDescriptor];

        [newRenderEncoder setRenderPipelineState:(__bridge id<MTLRenderPipelineState>)ctx->computeRenderPipelineState];
        [newRenderEncoder setDepthStencilState:(__bridge id<MTLDepthStencilState>)ctx->computeDepthStencilState];

        [newRenderEncoder setVertexBuffer:(__bridge id<MTLBuffer>)ctx->computeVertexBuffer offset:0 atIndex:0];
        [newRenderEncoder setVertexBuffer:instanceBuffer offset:0 atIndex:1];
        [newRenderEncoder setVertexBuffer:cameraBuffer offset:0 atIndex:2];

        [newRenderEncoder setFragmentTexture:(__bridge id<MTLTexture>)ctx->mandelbrotTexture atIndex:0];

        [newRenderEncoder setCullMode:MTLCullModeBack];
        [newRenderEncoder setFrontFacingWinding:MTLWindingCounterClockwise];

        [newRenderEncoder drawIndexedPrimitives:MTLPrimitiveTypeTriangle
                                     indexCount:ctx->computeIndexCount
                                      indexType:MTLIndexTypeUInt16
                                    indexBuffer:(__bridge id<MTLBuffer>)ctx->computeIndexBuffer
                              indexBufferOffset:0
        ...
        // STEP 7: DO NOT end the encoder - return it so MetalDriver can continue using it
        // MetalDriver will end it in its end() method
        
        metal_debug_log("render_compute_cubes_with_encoder() - SUCCESS, returning new encoder");
        return (vdynamic*)(__bridge_retained void*)newRenderEncoder;
    }
}
...
DEFINE_PRIM(_DYN, render_compute_cubes_with_encoder, _DYN _DYN);
